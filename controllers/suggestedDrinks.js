...
export const displaySuggestedDrink =  async (req, res) => {
  try {
    const { id } = req.params
    const singleDrink = await SuggestedDrink.findById(id).populate('owner').populate('comments.owner')
...
export const editSuggestedDrink =  async (req, res) => {
  const { id } = req.params
  try {
    const drinkToUpdate = await SuggestedDrink.findOneAndUpdate({ _id: id }, { ...req.body }, { new: true })
